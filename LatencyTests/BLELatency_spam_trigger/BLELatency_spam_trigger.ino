/*
   ...
   Constantly streams BLE MIDI messages (zero) until
   pin is toggled, in which case non-zero note is transmitted
   IMMEDIATELY

   Effective rate is (Streaming rate + 1) Hz due to the additional trigger
...
int send_interval = 8000; //default 10hz
int numIntervals = 10;
...
void loop() {
  ...
  if (deviceConnected)
  {
    ...
    currVal = digitalRead(D0);
    if (currVal == HIGH && preVal == LOW)
    {
      armed = true;      
    }
    if (armed)
